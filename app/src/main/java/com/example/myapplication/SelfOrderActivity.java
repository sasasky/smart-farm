...
public class SelfOrderActivity extends AppCompatActivity {
        ...
        mAdapter.setOnItemClickListener(new User2ListAdapter.OnItemClickListener() {
            ...
            public void onClick(int position) {
                ...
                if (Order.get(position).getOrderState()==order.State.unpaid){
                    intent.setClass(SelfOrderActivity.this, PaySelfActivity.class);
                    intent.putExtra("orderId",Order.get(position).getOrderId());
                }else {
                    intent.setClass(SelfOrderActivity.this, SelfInfoActivity.class);
                    intent.putExtra("orderId",Order.get(position).getOrderId());
                }
    ...
    public void requestSelf() {
        ...
        Call<orderlist> call = request.getSelf(userId);
        call.enqueue(new Callback<orderlist>() {
            @Override
            public void onResponse(Call<orderlist> call, Response<orderlist> response) {
                List<order> orders=response.body().getData();
                if(orders==null){
                    Toast.makeText(SelfOrderActivity.this, "没有订单", Toast.LENGTH_SHORT).show();
                }
                Order.addAll(orders);
